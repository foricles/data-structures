...
#if _DEBUG
	static void PrintProt(const std::string &str, Node* node, bool isLeft);
#endif // _DEBUG
...
{
#if _DEBUG
	std::cout << "added: " << m_size << std::endl;
#endif // _DEBUG
...
#if _DEBUG
	std::cout << "added: " << m_size << std::endl << "==================" << std::endl;
#endif // _DEBUG
}
...
#if _DEBUG
template<class Data>
inline void RBTree<Data>::PrintProt(const std::string &str, Node* node, bool isLeft)
{
	std::cout << str << "|__";
	if (node != nullptr)
	{
		...
		PrintProt(str + (isLeft ? "|   " : "    "), node->right, true);
		PrintProt(str + (isLeft ? "|   " : "    "), node->left, false);
	}
